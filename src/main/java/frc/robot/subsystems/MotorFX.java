...
public class MotorFX extends SubsystemBase implements MotorDef {
  ...
  public void logPeriodic() {
    ...
    if (pos != lastPos) {
      ...
      if (followId > 0) {
        logf(
            "%s motor cur:%.2f temp:%.2f vel:%.2f pos:%.0f inv:%b senP:%b\n",
            name,
            motor.getStatorCurrent().getValue(),
            motor.getDeviceTemp().getValue(),
            motor.getVelocity().getValue(),
            pos,
            motorInvert,
            sensorPhase);
        logf(
            "%s follow  cur:%.2f temp:%.2f vel:%d pos:%.2f\n",
            name,
            followMotor.getStatorCurrent(),
            followMotor.getDeviceTemp().getValue(),
            followMotor.getVelocity().getValue(),
            followMotor.getPosition().getValue());
      } else {
        logf(
            "%s motor cur:%.2f temp:%.2f vel:%d pos:%d inv:%b senP:%b\n",
            name,
            motor.getStatorCurrent().getValue(),
            motor.getDeviceTemp().getValue(),
            motor.getVelocity().getValue(),
